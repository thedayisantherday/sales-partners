...
import { getYears, getMonths, getDays } from '../datetime/makeData'
...
DatetimePicker.prototype = {
      ...
      each(TYPE_MAP, function (type) {
        ...
        self[type + 'Scroller'] = renderScroller(div, data, trimZero(newValueMap[type]), function (currentValue) {
          ...
          if (type === 'year') {
            ...
            if (self.config.extraObj && self.config.extraObj[0] && self.config.extraObj[0].length && self.config.extraObj[0].some(item => item.value === currentValue)) {
              self._setMonthScrollerNull()
              self._setDayScrollerNull()
            } else {
            ...
            currentDay = self.dayScroller.value
            if (self.config.extraObj[1] && self.config.extraObj[1] && self.config.extraObj[1].length && self.config.extraObj[1].some(item => item.value === currentValue)) {
              self._setDayScrollerNull()
            } else {
  ...
  _makeData (type, year, month) {
    ...
    if (this.config.extraObj) {
      switch (type) {
        case 'year':
          if (this.config.extraObj[0] && this.config.extraObj[0].length) {
            for (let item in this.config.extraObj[0]) {
              data.push(this.config.extraObj[0][item])
            }
          }
          break
        case 'month':
          if (this.config.extraObj[1] && this.config.extraObj[1].length) {
            for (let item in this.config.extraObj[1]) {
              data.push(this.config.extraObj[1][item])
            }
          }
          break
        case 'day':
          if (this.config.extraObj[2] && this.config.extraObj[2].length) {
            for (let item in this.config.extraObj[2]) {
              data.push(this.config.extraObj[2][item])
            }
          }
          break
        case 'hour':
          if (this.config.extraObj[3] && this.config.extraObj[3].length) {
            for (let item in this.config.extraObj[3]) {
              data.push(this.config.extraObj[3][item])
            }
          }
          break
        case 'minute':
          if (this.config.extraObj[4] && this.config.extraObj[4].length) {
            for (let item in this.config.extraObj[4]) {
              data.push(this.config.extraObj[4][item])
            }
          }
          break
      }
  ...
  _setMonthScrollerNull () {
    const self = this
    this.monthScroller.destroy()
    var div = self.find('[data-role=month]')
    self.monthScroller = renderScroller(div, [], '', null)
  },
  // after year change
  ...
  _setDayScrollerNull () {
    ...
    self.dayScroller = renderScroller(div, [], '', null)
  },
